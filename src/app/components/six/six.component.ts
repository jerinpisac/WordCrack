...
import { sixLetter } from '../../shared/six.model';
...
export class SixComponent implements OnInit{
  ...
  result: string = "";
  idx: number = 0;
  sixLetterList: string[] = sixLetter;

  guesses: string[][] = Array(6).fill(null).map(() => Array(6).fill(""));
  colors: string[][] = Array(6).fill(null).map(() => Array(6).fill(""));
  ...
  ngOnInit(): void {
    const storedIndex = localStorage.getItem("currentIndex6");
    if (storedIndex) {
      this.idx = parseInt(storedIndex);
    } else {
      this.idx = Math.floor(Math.random() * this.sixLetterList.length);
      localStorage.setItem("currentIndex6", this.idx.toString());
    }
    this.result = this.sixLetterList[this.idx];
  }
  ...
  isRight() {
    ...
    else if (this.sixLetterList.includes(this.word)) {
